...
test('styles.css 包含 sticky 顶部、版本徽标和新的字号契约', () => {
  ...
  assert.match(styles, /\.page-sidebar\s*\{[\s\S]*overflow-x:\s*hidden;[\s\S]*overflow-y:\s*auto;/);
  assert.match(styles, /\.section-switcher\s*\{[\s\S]*top:\s*var\(--sticky-shell-top,\s*1rem\);/);
  assert.match(styles, /\.section-switcher\s*\{[\s\S]*overflow-x:\s*hidden;[\s\S]*overflow-y:\s*auto;/);
